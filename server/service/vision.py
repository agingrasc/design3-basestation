...
class VisionWebSocketHandler(websocket.WebSocketHandler):
    ...
    def initialize(self, global_state, robot_position, tasks):
    ...
    def on_message(self, message_data):
        ...
        if message_type == "register_task_data":
            ...
            notify_all(REGISTERED_TO_TASK_DATA, self._tasks.to_dto())

        if message_type == "register_robot_cycle":
            register_to(REGISTERED_TO_ROBOT_CYCLE, self)
        ...
        if message_type == "reset_tasks":
            self._tasks.reset_all()
            notify_all(REGISTERED_TO_TASK_DATA, self._tasks.to_dto())

        if message_type == "new_round":
            self._tasks.reset_all()

            self._tasks.mark_done('initial_orientation')
            self._tasks.mark_done('identifie_antenna')
        ...
        if message_type == "cycle_started":
            notify_all(REGISTERED_TO_ROBOT_CYCLE, message)
    ...
    def update_tasks(self, message):
        ...
        self._tasks.mark_done(message_data["task_name"])
        notify_all(REGISTERED_TO_TASK_DATA, self._tasks.to_dto())
...
def unregister(connection):
    ...
    if connection in REGISTERED_TO_ROBOT_CYCLE:
        REGISTERED_TO_ROBOT_CYCLE.remove(connection)
...
if __name__ == "__main__":
    ...
    application = web.Application([
        web.url(r"/", VisionWebSocketHandler,
                kwargs={'global_state': WORLD_STATE, 'robot_position': ROBOT_POSITION, 'tasks': tasks})
    ])
